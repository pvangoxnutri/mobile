...
export interface Country {
  ...
  name: string;
  continent: Continent;
  /** Second continent for transcontinental countries (e.g. Turkey, Russia) */
  continent2?: Continent;
  /** Hardcoded flag emoji — overrides the auto-derived one for subdivision codes */
  flag?: string;
}

export interface CountryEntry extends Country {
  status: CountryStatus;
}

export type StatusMap = Record<string, CountryStatus>;

/** Derives flag emoji from a 2-letter ISO country code */
...
/** Returns the correct flag emoji for any country, including subdivision entries */
export function getCountryFlag(country: Country): string {
  return country.flag ?? getFlagEmoji(country.code);
}

/** Returns true if the country belongs to the given continent (primary or secondary) */
export function inContinent(country: Country, continent: Continent): boolean {
  return country.continent === continent || country.continent2 === continent;
}
...
export const COUNTRIES: Country[] = [
  ...
  { code: 'RU', name: 'Russia', continent: 'Europe', continent2: 'Asia' },
  ...
  { code: 'GB-ENG', name: 'England', continent: 'Europe', flag: '🏴󠁧󠁢󠁥󠁮󠁧󠁿' },
  { code: 'GB-SCT', name: 'Scotland', continent: 'Europe', flag: '🏴󠁧󠁢󠁳󠁣󠁴󠁿' },
  { code: 'GB-WLS', name: 'Wales', continent: 'Europe', flag: '🏴󠁧󠁢󠁷󠁬󠁳󠁿' },
  { code: 'GB-NIR', name: 'Northern Ireland', continent: 'Europe', flag: '🇬🇧' },
...
export function buildEntries(statusMap: StatusMap): CountryEntry[] {
  return COUNTRIES.map((c) => ({ ...c, status: statusMap[c.code] ?? 'none' }));
}
